...
ssize_t razer_attr_write_side_mode_static(IOUSBDeviceInterface **usb_dev, const char *buf, size_t count, int side)
{
    ...
    if(count == 3) {
        switch(product) {
            ...
            case USB_DEVICE_ID_RAZER_BASILISK_V3:
            case USB_DEVICE_ID_RAZER_BASILISK_V3_X_HYPERSPEED:  // Add this line
...
ssize_t razer_attr_write_scroll_mode_wave(IOUSBDeviceInterface **usb_dev, const char *buf, size_t count)
{
    ...
    switch(product) {
        ...
        // Add these definitions near the other USB device ID definitions:
        ...
        default:
            printf("razermouse: scroll_mode_wave not supported for this model\n");
            return count;
    }
